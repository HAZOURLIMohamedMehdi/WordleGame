...
public class Easybot extends Bot {
	...
	public void banWords(String lastGuess, String lastGuessResult) {
	    Iterator<String> iterator = this.wordsPossible.iterator();
	    while (iterator.hasNext()) {
	        ...
	        for (int i = 0; i < lastGuessResult.length(); i++) {
	            ...
	            else if (resultChar == '%' && word.indexOf(guessChar) == -1) {
	                iterator.remove();
	                break;
	            }
	        }
	...
	public void playAgainstEasyBot(int numberOfLetters, String wordToGuess) {
	...
	public int playAgainstEasyBotAnalysis(int numberOfLetters, String wordToGuess) {
		System.out.println("Tentative initiale aléatoire");
		this.playFirstTry(numberOfLetters);
		//System.out.println(this.wordsPossible);
	...
	public ArrayList<Integer> botAnalysis(int numberOfGames, int numberOfLetters)
	{
		...
		while (numberOfGames>=0)
		{
			...
			analysis.add(playAgainstEasyBotAnalysis(numberOfLetters, randomWord)) ;		
			numberOfGames--;
		}
